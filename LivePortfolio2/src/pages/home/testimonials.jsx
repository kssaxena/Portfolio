...
const Testimonials = () => {
  ...
  const Card = ({
    ...
    return (
      <div className="h-fit w-full rounded-lg flex flex-col m-5 justify-evenly items-start select-none gap-5">
        {/* DESCRIPTION */}
        <ScrollRevealText
          textParts={splitDescriptionToParts(description)}
          className="text-[52px] leading-[1.1] tracking-tighter text-justify"
        />
  ...
  const Carousel = () => {
    ...
    return (
      <div className="flex justify-end items-start w-full gap-20">
        <span className="text-neutral-400 font-dmMono">
          {current + 1}/<span className="text-[#ff5533]">{totalSlides}</span>
        </span>
        <div className="overflow-hidden">
          <div
            className="flex transition-transform duration-700 ease-in-out rounded-xl w-[130vh]"
        ...
        <div className="">
          <div className="flex flex-col justify-center gap-2 my-5 lg:mt-20">
            {testimonials.map((item, index) => (
              <button
                key={item.id}
                onClick={() => setCurrent(index)}
                className={`h-16 w-16 rounded-full border overflow-hidden flex items-center justify-center
        ${
          index === current
  ...
  return (
    <div className="flex flex-col justify-start items-start gap-36 border-t border-gray-800 py-10 px-5 text-neutral-200 text-[18px] relative min-h-screen">
